			...
			vim.schedule(function()
				...
				if status_output and #status_output > 0 then
					...
					count_changed_lines(function(lines_changed)
						...
						-- TODO: Rewrite this to prompt for files one by one...
						-- if #unignored_files > 0 then
						for file, checked in pairs(unignored_files) do
							notify("{" .. file .. "," .. tostring(checked) .. "}")
							if not checked then
								vim.ui.select({ "Add to git", "Add to .gitignore", "Skip" }, {
									prompt = "Untracked files not in .gitignore: (" .. file .. ") ",
								}, function(choice)
									...
									elseif choice == "Add to .gitignore" then
										Job:new({
											command = "sh",
											args = { "-c", "echo '" .. "/" .. file .. "' >> .gitignore" },
										}):start()
										table.remove(unignored_files)
									else
...
function M.start_watcher()
	...
	Job:new({
		...
		on_stdout = vim.schedule_wrap(function(_, data)
			if data then
				notify("[GitWatcher] on_stdout received data: " .. data, vim.log.levels.DEBUG)
				-- Explicitly close the previous timer if it exists and is active
				if current_debounce_timer and current_debounce_timer:is_active() then
					notify("[GitWatcher] Closing existing timer", vim.log.levels.DEBUG)
					current_debounce_timer:stop()
					current_debounce_timer:close()
				...
				-- Schedule the new check and store its timer object
				notify("[GitWatcher] Scheduling debounced_check", vim.log.levels.DEBUG)
				current_debounce_timer = vim.defer_fn(debounced_check, debounce_interval)
			end
		end),
	}):start()
end
